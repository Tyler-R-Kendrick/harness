...
import { bytes, cascadePolicy, decideToolCalls, Ensemble, probability } from "@harness/cognitive";
    ...
    e.register(d("judge-a", ["judgment"], ["judge"], "hosted"), async () => ({
      judge: {
        evaluate: async (r) => {
          ...
          return { correct: options.judgeAnswer ?? { type: "boolean" as const, probability: probability(options.judgeP!) } };
        },
      },
    }));
...
const weather: Routing = { calls: [{ name: "get_weather", arguments: { city: "Lagos" } }], confidence: probability(0.97), reasoning: "city named" };
...
describe("tool-call cascade: router, then judge, then generator", () => {
  it("CA1.1 a confident, valid routing is taken as is", async () => {
    ...
    expect(decision).toMatchObject({ calls: weather.calls, decidedBy: "router", confidence: probability(0.97) });
  ...
  it("CA1.2 a middling routing is checked by the judge and kept when the judge agrees", async () => {
    const { e, calls } = setup({ routing: { ...weather, confidence: probability(0.7) }, judgeP: 0.92, generated: "" });
    const decision = await decideToolCalls(e, { input: "weather in Lagos?", tools });
    expect(decision).toMatchObject({ calls: weather.calls, decidedBy: "router+judge", confidence: probability(0.92) });
  ...
  it("CA1.3 when the judge disagrees the generator decides", async () => {
    const { e, calls } = setup({ routing: { ...weather, confidence: probability(0.7) }, judgeP: 0.3, generated: "" });
  ...
  it("CA1.4 a low-confidence routing skips the judge and escalates", async () => {
    const { e, calls } = setup({ routing: { ...weather, confidence: probability(0.2) }, judgeP: 0.99, generated: "" });
  ...
  it("CA1.5 a routed call to an unknown tool, or one missing a required argument, escalates whatever its confidence", async () => {
    for (const bad of [
      ...
      const { e } = setup({ routing: { calls: [bad], confidence: probability(1), reasoning: "" }, judgeP: 0.99, generated: "" });
  ...
  it("CA1.6 with no generator the router's answer stands, flagged unverified", async () => {
    const { e } = setup({ routing: { ...weather, confidence: probability(0.2) }, generated: null });
  ...
  it("CA1.8 an empty, confident routing means no tool applies", async () => {
    const { e } = setup({ routing: { calls: [], confidence: probability(0.96), reasoning: "no poem tool" }, generated: "" });
  ...
  it("CA2.1 the trace records each step exactly: who answered, with what confidence, and why it moved on", async () => {
    ...
    expect((await decideToolCalls(setup({ routing: { ...weather, confidence: probability(0.7) }, judgeP: 0.92 }).e, { input: "x", tools })).trace).toEqual([
      { step: "route", member: "router-a", outcome: "confidence 0.7" },
      { step: "verify", member: "judge-a", outcome: "p=0.92" },
    ]);
    const unknown = await decideToolCalls(setup({ routing: { calls: [{ name: "launch_rocket", arguments: {} }], confidence: probability(1), reasoning: "" }, generated: "" }).e, { input: "x", tools });
    expect(unknown.trace[0]).toEqual({ step: "route", member: "router-a", outcome: "invalid: unknown tool launch_rocket" });
    const missing = await decideToolCalls(setup({ routing: { calls: [{ name: "get_weather", arguments: {} }], confidence: probability(1), reasoning: "" }, generated: "" }).e, { input: "x", tools });
    ...
    expect((await decideToolCalls(setup({ routing: { ...weather, confidence: probability(0.7) }, generated: "" }).e, { input: "x", tools })).trace).toEqual([
      { step: "route", member: "router-a", outcome: "confidence 0.7" },
  ...
  it("CA2.3 thresholds are inclusive: confidence equal to act is taken, equal to verify is judged, and p equal to accept is kept", async () => {
    ...
    expect((await decideToolCalls(setup({ routing: { ...weather, confidence: probability(0.9) }, judgeP: 0, generated: "" }).e, { input: "x", tools }, policy)).decidedBy).toBe("router");
    expect((await decideToolCalls(setup({ routing: { ...weather, confidence: probability(0.5) }, judgeP: 0.8, generated: "" }).e, { input: "x", tools }, policy)).decidedBy).toBe("router+judge");
  ...
  it("CA2.4 a judge that answers some other way, or not at all, counts as p=0", async () => {
    for (const judgeAnswer of [{ type: "score" as const, score: 1 }, null]) {
      const decision = await decideToolCalls(setup({ routing: { ...weather, confidence: probability(0.7) }, judgeAnswer, generated: "" }).e, { input: "x", tools });
      expect(decision.decidedBy).toBe("generator");
  ...
  it("CA2.5 with no generator only the router's valid calls stand; with neither router nor generator there is no member", async () => {
    const mixed: Routing = { calls: [...weather.calls, { name: "launch_rocket", arguments: {} }], confidence: probability(0.99), reasoning: "" };
