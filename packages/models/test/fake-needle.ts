import type { NeedleModule } from "@harness/models";
...
/**
 * Emulates the Emscripten module Needle 3 ships (wasm/needle.js): a flat heap, a
 * bump allocator and the C API from needle.h, reached through ccall.
 */
export class FakeNeedleModule implements NeedleModule {
  ...
  dimensions = 4;
  reply: (input: string, tools: string) => NeedleReply = () => ({ success: true, function_calls: [], reasoning: "", confidence: 1 });
  #next = 16;
  #tools = "";
  ...
  _needle_load(_ptr: number, n: bigint): number {
