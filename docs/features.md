...
| SDK harness and ACP-agent workers: any AI SDK `HarnessAgent` (Claude Code, Codex, or any ACP agent through `@ai-sdk/harness-acp`) runs through the agent worker via `harnessSessions`: one harness session per daemon session, started on its first turn, given only each turn's new prompt; its calls of host tools run on the host, tool approvals go through the daemon's permission flow, cancel aborts the turn. On the native host (`--worker harness --harness claude-code|codex|acp:<package>@<version>:<executable>`) each session runs in a sandbox of its own: a host sandbox (`hostSandbox`: a directory and a loopback port on this machine, unisolated) or, with `--sandbox docker:<image>`, a Docker container (`dockerSandbox`: files and processes isolated from this machine, the port published on its loopback, an optional setup command and environment); shutdown stops harness sessions (their bridges and sandbox processes end) and parks them in a file (`--harness-state`), and a restarted daemon resumes them, starting fresh when a parked state cannot be resumed | built | HS1.1–HS1.11, HS2.1–HS2.2, TH1.1–TH1.4, HX1.1–HX1.7, DS1.1–DS1.8, HH1.1–HH1.7, HI1.1–HI1.3 (the official ACP adapter and its bridge in a host sandbox, end to end, including a resume after a daemon restart, and in a Docker container); the ACP adapter installs its bridge with pnpm, which must be on PATH (in a container, the setup command can install it) |
...
| Agent worker: any AI SDK `Agent` as a session worker (a `ToolLoopAgent` from `sessionAgent` over the ensemble, or over a gateway model); it keeps each session's conversation, streams text, reasoning and tool calls as ACP updates, and turns with images go to the vision model; a failed model call is an error notice and ends the turn as a refusal, never as a finished turn | built | AW1.1–AW1.6, AW1.9, AW1.12, HS1.8; CLI `--worker ensemble`, `--worker model` |
...
| Browser host for the ensemble: `buildBrowserEnsemble` registers every catalog model that runs in a browser with the same runtime loaders as the native host (`portableLoaders` in `@harness/models`), verified files kept in the Cache API (`CacheStorageByteCache`), transformers.js on WebAssembly or WebGPU (`device`), constrained decoding with XGrammar from its bundled source (`xgrammarFromSource`); without an XGrammar loader no model claims to enforce constraints | built | BE1.1–BE1.11, BI3.1–BI3.3 (Chromium: the Cache API, XGrammar compiling and recovering, a Cactus WASM router answering a cognitive invoke over ACP); Emscripten and XGrammar evaluate their loaders, which an extension's CSP forbids; no OPFS |
