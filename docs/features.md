...
| Skill builder: an agent skill (SKILL.md with name/description frontmatter within the spec's limits) whose procedure is a durable workflow it tells the agent to run and resume | built | LP3.1–LP3.2 |
| Tool builder (code mode): a model writes the tool as workflow code composing the available tools and model questions; each draft is checked (compiles, defines the workflow, calls only tools that exist) and failures go back to the model; the tool is learned and runs as a durable workflow | built | LP4.1–LP4.4, CH3.4 |
...
| Durable workflows as code (`@harness/workflows`): deterministic workflow code in a QuickJS WebAssembly sandbox (no clock, randomness, network, files or timers; counted CPU budget, memory limit); every effect (`ctx.tool`, `ctx.ask`) is journaled before the code sees it, so an interrupted run resumes by replay and a finished run returns its result; a journal that diverges from the code is refused | built | WF1.1–WF1.9; ADR 0002 |
| Workflow library and host: workflows kept by name (one file each natively, run journals beside them), other workflows callable as tools (nested durable runs), `workflows.list/get/run` through the cognitive invoke operation, `harness-workflow run <file> --run <id>` CLI | built | WH1.1–WH1.5, WX1.1–WX1.2, WC1.1–WC1.2, CH3.4; CLI `--workflows <dir>` |
...
| Plugins offered by ACP clients (bridged over the capability registry), automatic reflection when a session ends, lesson and plugin evals on real models, client tools inside workflows | not started | Plugins are in-process today; sessions are observed through `learning.observe`; workflow tools are library workflows unless the host passes a tool executor |
